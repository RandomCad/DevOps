...
from fuchs import DatabaseConnection, URL_HAMSTER, DB_NAME, DB_USER, \
DB_PASSWORD, DB_HOST  # , URL_CHAMELEON

app = FastAPI()

def get_db():
    """returns a database connection"""
    db = DatabaseConnection(dbname=DB_NAME, user=DB_USER, password=DB_PASSWORD, host=DB_HOST)
    try:
        yield db
    finally:
        db.conn.close()
...
def read_note(note_id: int, db: DatabaseConnection = Depends(get_db)):
    ...
    pics = [
        {"id": pic[0], "name": pic[1], "alt_text": pic[2]}
        for pic in pic_data
    ]
    return {"id": note_id, "content": note_content_md, "pictures": pics}

@app.post("/notes/")
def create_note(note_title: str, note_content_md: str, db: DatabaseConnection = Depends(get_db)):
    """creates a new note.\n
...
def update_note(note_id: int, note_title: str, note_content_md: str, db: DatabaseConnection = Depends(get_db)):
...
def store_picture(note_id: int, pic_content: str, pic_name: str, pic_alt_text: str, db: DatabaseConnection = Depends(get_db)):
    ...
    pic_id = db.store_meta_of_picture(
        note_id, pic_name, pic_alt_text, pic_path)

    return {"status": "created", "id": pic_id, "path": pic_path}

@app.put("/notes/{note_id}/pics/{pic_path}")
def update_picture(note_id: int, pic_id: int, pic_content: str, pic_name: str, pic_alt_text: str, db: DatabaseConnection = Depends(get_db)):
...
@app.delete("/notes/{note_id}/pics/{pic_path}")
